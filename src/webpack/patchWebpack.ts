...
function patchFactory(moduleId: PropertyKey, originalFactory: AnyModuleFactory): PatchedModuleFactory {
    ...
    // Expose declarations
    if (!wasClassModule) {
        const vars = parseDeclarations(originalFactoryCode);
        const functionBody = patchedCode.indexOf(")") + 2;
        ...
        patchedSource = `${patchedCode.slice(0, functionBody)};arguments[0].declarations=${declarationString};${patchedCode.slice(functionBody)}\n//# sourceURL=${sourceUrl}`;
